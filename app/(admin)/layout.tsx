...
export default function AdminLayout({
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {!isCampaignsPage && (
        <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-border flex items-center justify-between gap-2 px-3 py-2.5 safe-area-inset-top">
          <Link href="/dashboard" className="min-w-0 flex-1">
            <h2 className="text-lg font-bold truncate">Dark Velvet</h2>
          </Link>
      ...
      {open && (
        <div
          className="fixed inset-0 z-40 bg-gray-900 text-white p-4 space-y-5 pt-16 pb-8 overflow-y-auto overscroll-contain"
          role="dialog"
          aria-modal="true"
        >
      ...
      <main
        className={`flex-1 min-w-0 bg-gray-50 relative ${
          isCampaignsPage ? "overflow-hidden" : "overflow-y-auto overflow-x-hidden"
        }`}
      >
        <div
          className={`min-h-full min-w-0 w-full ${
            isCampaignsPage ? "h-full" : "pt-14 md:pt-0 px-3 sm:px-4 md:px-6 py-4 md:py-6"
          }`}
        >
          {children}
        </div>
