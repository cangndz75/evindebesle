...
import { bestSellersMen } from "@/lib/homeData";
...
export default function MenBestSellersPage() {
  ...
  // Convert homeData products to local Product type
  const products: Product[] = bestSellersMen.map((p) => ({
    id: p.id,
    name: p.title,
    price: p.price,
    ...
    <div className="min-h-screen bg-white pt-[65px] md:pt-[81px]">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        <nav className="mb-4">
          ...
          <span className="text-sm text-[#111]/60 font-light mx-2">/</span>
          <span className="text-sm text-[#111] font-light">En Çok Satanlar</span>
        </nav>
              ...
              <div className="mt-8 space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {["Black", "White", "Navy", "Gray", "Beige", "Brown"].map((color) => (
                      <button
                        key={color}
                        className="w-10 h-10 rounded-full border border-gray-300"
                        style={{ backgroundColor: color.toLowerCase() === "white" ? "#fff" : color.toLowerCase() === "beige" ? "#F5F5DC" : color.toLowerCase() }}
                      />
                    ))}
                  </div>
                </div>
