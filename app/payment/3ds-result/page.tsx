...
export default function Payment3DSResultPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/payment/session?id=${sid}`, {
          cache: "no-store",
        });
        if (res.ok) {
    ...
    <div className="min-h-[70vh] flex flex-col items-center justify-center p-6">
      {isSuccess ? (
        ...
        <>
          ...
          {errorDetail && (
            <div className="bg-red-50 text-red-700 px-4 py-2 rounded mb-6 max-w-md w-full text-sm">
              {errorDetail}
            </div>
          )}
          <Button asChild>
