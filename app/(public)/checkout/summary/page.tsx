                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500 mt-1">
                                                    {item.size?.name} {item.color?.name && `â€¢ ${item.color.name}`}
                                                </p>
                                            </div>
                                            <button
                                        ...
                                        <div className="flex justify-between items-end mt-4">
                                            ...
                                            <div className="flex flex-col items-end">
                                                <p className="font-medium text-lg">{(item.product.price) * item.quantity} â‚º</p>
                                                {item.product.originalPrice && item.product.originalPrice > item.product.price && (
                                                    <p className="text-sm text-gray-400 line-through">
                                                        {item.product.originalPrice * item.quantity} â‚º
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white p-6 rounded-lg shadow-sm sticky top-24">
                            ...
                            <div className="space-y-3 text-sm text-gray-600">
                                ...
                                <div className="flex justify-between">
                                    <span>Kargo</span>
                                    <span>{shipping === 0 ? "Ãœcretsiz" : `${shipping.toFixed(2)} â‚º`}</span>
                                </div>
                                {discount > 0 && (
                                    <div className="flex justify-between text-green-600">
                                        <div className="flex items-center gap-2">
                                            <span>Ä°ndirim ({couponCode})</span>
                                            <button
                            ...
                            {subtotal < freeShippingThreshold ? (
                                <div className="mt-4 p-3 bg-purple-50 text-purple-700 text-xs rounded border border-purple-100">
                                    Ãœcretsiz kargo hakkÄ± kazanmanÄ±za {(freeShippingThreshold - subtotal).toFixed(2)} TL kaldÄ±!
                                </div>
                            ) : (
                                <div className="mt-4 p-3 bg-green-50 text-green-700 text-xs rounded border border-green-100">
                                    âœ“ Ãœcretsiz kargo hakkÄ± kazandÄ±nÄ±z!
                                </div>
                            )}
                        </div>
                    </div>
